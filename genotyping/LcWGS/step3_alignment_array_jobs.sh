...
reference_genome=$(head -n 19 ${pipeline_arguments} | tail -n 1)
...
#### used to gather the register group info that bwa mem needs
...
fastq_prefix=$(ls ${trimmed_data}/${sample2}_trimmed_R1.fastq.gz | rev | cut -d'/' -f 1 | cut -d '_' -f 2- | rev)
fastq_prefix=${fastq_prefix%_trimmed}
...
#### mem : bwa-mem algorithm: seeds alignments with maximal exact matches (MEMs), extends seeds with the affine-gap Smith-Waterman algorithm
#### R : group header line
#### a : outputs all found alignments for unpaired PE reads, flags them as secondary alignments
...
#### check for successfully mapped reads
